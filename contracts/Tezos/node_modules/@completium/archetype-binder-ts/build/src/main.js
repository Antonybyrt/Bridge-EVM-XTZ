...
const mich_to_event_body = (name, fields, mty, arg, ci) => {
    ...
    const h = (a, n) => { return typescript_1.factory.createElementAccessExpression(a, typescript_1.factory.createNumericLiteral(n)); };
    const g = (a) => {
        return typescript_1.factory.createPropertyAccessExpression(typescript_1.factory.createAsExpression(a, typescript_1.factory.createTypeReferenceNode(typescript_1.factory.createQualifiedName(typescript_1.factory.createIdentifier("att"), typescript_1.factory.createIdentifier("Mpair")), undefined)), typescript_1.factory.createIdentifier("args"));
    };
    let args = [];
    let ids = fields.map(x => x.name);
    const new_ids = extract_id(mty);
    ...
    if (is_three) {
        args.push((0, utils_1.mich_to_archetype_type)(fields[0].type, h(g(arg), 0), ci));
        args.push((0, utils_1.mich_to_archetype_type)(fields[1].type, h(g(h(g(arg), 1)), 0), ci));
        args.push((0, utils_1.mich_to_archetype_type)(fields[2].type, h(g(h(g(arg), 1)), 1), ci));
    }
    else {
        for (let i = 0; i < fields.length; ++i) {
            const field = fields[i];
...
const eventToInterfaceDecl = (r, ci) => {
    const mty = r.type_michelson;
    const mich_body = mich_to_event_body(r.name, r.fields, mty, typescript_1.factory.createIdentifier("input"), ci);
    return [entityToInterfaceDecl(r.name, mty, r.fields, mich_body, ci)];
};
